...
def register():
    ...
    if is_api_request:
        ...
        db.session.commit()
        print(f"Is API Request: {is_api_request}, Received Data: {data}")
        print("Returning JSON response for register")
        return jsonify({'message': 'User registered successfully'}), 201
    else:
        # Handle non-API requests (if any) - currently redirects to index
...
def view_member(member_id):
    ...
    # If the request is an API request, return JSON
    if request.headers.get('X-Requested-With') == 'XMLHttpRequest' or request.is_json: # This line seems to be incorrectly indented. Moving it to the correct level.
        return jsonify({
 'id': family_member.id,
